...
/**
 * A {@link BugChecker} which flags arguments to {@link Optional#orElse(Object)} that should be
...
// flagged.
// XXX: We could also leave the method reference cleanup to `MethodReferenceUsageCheck` and express
// this logic using a Refaster template, i.c.w. a `@Matches` constraint which implements the
// `requiresComputation` logic. This does require that `MethodReferenceUsageCheck` becomes generally
// usable.
...
public final class OptionalOrElse extends BugChecker implements MethodInvocationTreeMatcher {
  ...
  private static final Matcher<ExpressionTree> OPTIONAL_OR_ELSE_METHOD =
      instanceMethod().onExactClass(Optional.class.getName()).namedAnyOf("orElse");
  // XXX: We might also want to exclude invocations of `@Placeholder`-annotated methods.
  private static final Matcher<ExpressionTree> REFASTER_METHOD =
      staticMethod().onClass("com.google.errorprone.refaster.Refaster");
